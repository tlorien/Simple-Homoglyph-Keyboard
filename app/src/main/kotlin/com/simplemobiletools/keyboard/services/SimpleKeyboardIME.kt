...
<<<<<<< HEAD
import com.simplemobiletools.keyboard.helpers.HomoglyphMappings
...
<<<<<<< HEAD
                val originalChar = code.toChar().toString()
                val originalText = inputConnection.getExtractedText(ExtractedTextRequest(), 0)?.text

                // Handle uppercase transformations if the shift state is active
                var transformedChar = if (Character.isLetter(originalChar.single()) && keyboard!!.mShiftState > ShiftState.OFF) {
                    if (baseContext.config.keyboardLanguage == LANGUAGE_TURKISH_Q) {
                        originalChar.uppercase(Locale.forLanguageTag("tr"))
                    } else {
                        originalChar.uppercase(Locale.getDefault())
                    }
                } else {
                    originalChar
                }

                // Next, replace the character with a homoglyph if the feature is enabled
                Log.d("KeyboardIME", "Original char: $transformedChar")
                if (config.homoglyphicsEnabled && HomoglyphMappings.mappings.containsKey(transformedChar.single())) {
                    transformedChar = HomoglyphMappings.mappings[transformedChar.single()]?.random() ?: transformedChar
                    Log.d("KeyboardIME", "Mapped char: $transformedChar")
=======
...
<<<<<<< HEAD
                            val fullText = originalText.toString() + transformedChar
=======
